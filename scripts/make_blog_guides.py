...
def bt_block(cat, label):
    """백테스트 결과. 나쁘면 나쁜 대로 적되, 자기 것을 깎지는 않는다."""
    ...
    out = [
        ...
        "",
        # 숫자는 위 표에 다 있다. 여기서는 그 숫자가 무엇을 뜻하는지만
        # 적는다. "제가 만든 건데도 못했습니다" 처럼 자기 것을 깎는 말로
        # 받으면, 읽는 사람이 "그럼 이걸 왜 쓰나" 하는 자리에서 답을 못
        # 듣고 끝난다. 가리자는 것이 아니라 잰 것을 그대로 말하자는 것이다.
        f"**{label} 조건만 보고 그냥 버티는 것으로는 이 구간에서 지수를 "
        f"넘지 못했습니다.** 숨기지 않고 그대로 적습니다.",
        "",
        "규칙을 걸면 승률은 오히려 떨어집니다. 자주 잘리기 때문입니다. "
        "대신 크게 깨지는 일이 줄어듭니다. 한 번에 반토막이 나면 그다음이 없습니다.",
        "",
        "조건표가 하는 일은 볼 종목을 줄이는 데까지입니다. 계좌를 지키는 "
        "것은 그다음 규칙이 합니다.",
    ]
