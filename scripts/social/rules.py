...
# 막는다.
#
# 2026-09-20 종목명 4,300개에 전부 돌려 헛걸림 0개를 확인했다. 낱말 하나로
# 걸면 '좋은사람들'(033340) '참좋은여행'(094850) 같은 실제 상장사가 소리
# 없이 지워진다. 여기 있는 것이 전부 구절인 이유다. 항목을 더할 때도
# 판정하는 꼴로 넣고, 넣은 뒤 종목명에 한 번 돌려본다.
BANNED_TERMS = [
    "MA50", "MA150", "MA200", "MA20", "MA21", "RS70", "52w",
    # 21일선을 20일선으로 바꿨는데(2026-09-16) 글이 안 따라온 적이 있다.
    # 화면은 20일선으로 계산하는데 글만 21일선이라고 하면 읽는 사람이
    # 어느 쪽을 믿어야 할지 모른다.
    "21일선", "21일 평균선",
