    ...
    if u and not _stale(u, 4):
        ...
        if total and strong is not None:
            lines += [
                ...
                # 두 줄로 따로 적는다. 한 줄에 괄호로 묶으면 뒤엣것이
                # 앞엣것에 속한 것처럼 읽힌다. 실제로는 겹치되 어느 쪽도
                # 상대를 품지 않는다. 엄격은 추세 8개 전부, 우량은 추세
                # 6개 이상에 실적까지다. 추세가 완벽해도 실적이 못 받쳐
                # 주면 엄격에만 들어가므로 엄격이 더 많은 날이 생긴다.
                # 그럴 때 괄호로 묶어 두면 "71개인데 그중 73개"가 되어
                # 읽는 사람이 숫자를 의심한다.
